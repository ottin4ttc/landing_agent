...
export async function patchPluginSessionExtension(params: {
  ...
  if (normalizedSlotKey?.ok === false) {
    log.warn(
      ...
      undefined,
      {
        event: "plugins.host.hook.state.plugin.session.extension.slot.promotion.skipped",
        outcome: "warning",
        reason: "skipped",
      },
    );
...
function projectSessionExtensionValue(params: {
  ...
  try {
    ...
    log.warn(
      ...
      {
        event:
          "plugins.host.hook.state.plugin.session.extension.projection.failed.plugin.namespace",
        outcome: "warning",
        reason: "failed",
      },
    );
