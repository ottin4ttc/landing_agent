...
export async function runGatewayLoop(params: {
  ...
  const writeStabilityBundle = async (reason: string, error?: unknown) => {
    ...
    if ("message" in result) {
      gatewayLog.warn(result.message, undefined, {
        event: "gateway.writestabilitybundle",
        outcome: "warning",
        reason: "warning",
      });
    }
  ...
  const reacquireLockForInProcessRestart = async (): Promise<boolean> => {
    try {
      ...
      gatewayLog.error(
        ...
        undefined,
        {
          event: "gateway.failed.reacquire.gateway.lock.process.restart",
          outcome: "failure",
          reason: "failed",
        },
      );
  ...
  const handleRestartAfterServerClose = async (restartReason?: string) => {
    ...
    if (isUpdateRestart) {
      ...
      if (respawn.mode === "spawned") {
        ...
        if (healthy) {
          gatewayLog.info(
            ...
            undefined,
            {
              event: "gateway.restart.mode.update.process.respawn.spawned.pid",
              outcome: "success",
              reason: "completed",
            },
          );
        ...
        gatewayLog.warn(
          ...
          undefined,
          {
            event: "gateway.update.respawn.child.did.not.become.healthy",
            outcome: "warning",
            reason: "warning",
          },
        );
        ...
        await markUpdateRestartSentinelFailure("restart-unhealthy").catch((err: unknown) => {
          gatewayLog.warn(
            `failed to mark update restart sentinel unhealthy: ${String(err)}`,
            undefined,
            {
              event: "gateway.failed.mark.update.restart.sentinel.unhealthy",
              outcome: "warning",
              reason: "failed",
            },
          );
      ...
      if (respawn.mode === "supervised") {
        ...
        gatewayLog.info("restart mode: update process respawn (supervisor restart)", undefined, {
          event: "gateway.restart.mode.update.process.respawn.supervisor.restart",
          outcome: "success",
          reason: "completed",
        });
      ...
      if (respawn.mode === "failed") {
        gatewayLog.warn(
          ...
          undefined,
          {
            event: "gateway.update.respawn.failed.falling.back.process.restart",
            outcome: "warning",
            reason: "failed",
          },
        );
        ...
        gatewayLog.info(
          ...
          undefined,
          {
            event: "gateway.restart.mode.process.restart",
            outcome: "success",
            reason: "completed",
          },
        );
    ...
    if (respawn.mode === "spawned" || respawn.mode === "supervised") {
      ...
      gatewayLog.info(`restart mode: full process restart (${modeLabel})`, undefined, {
        event: "gateway.restart.mode.full.process.restart",
        outcome: "success",
        reason: "completed",
      });
      if (supervisorMode === "launchd") {
    ...
    if (respawn.mode === "failed") {
      ...
      gatewayLog.warn(
        ...
        undefined,
        {
          event: "gateway.full.process.restart.failed.falling.back.process",
          outcome: "warning",
          reason: "failed",
        },
      );
      ...
      gatewayLog.info(
        `restart mode: in-process restart (${respawn.detail ?? "OPENCLAW_NO_RESPAWN"})`,
        undefined,
        { event: "gateway.restart.mode.process.restart", outcome: "success", reason: "completed" },
      );
    }
    if (!(await reacquireLockForInProcessRestart())) {
      return;
  ...
  const armPendingStartupForceExitTimer = () => {
    ...
    pendingStartupForceExitTimer = setTimeout(() => {
      ...
      gatewayLog.error(
        ...
        undefined,
        {
          event: "gateway.startup.restart.request.timed.out.before.gateway",
          outcome: "failure",
          reason: "timeout",
        },
      );
  ...
  const runAcceptedRequest = ({
    ...
    const armForceExitTimer = (forceExitMs: number) => {
      ...
      forceExitTimer = setTimeout(() => {
        gatewayLog.error("shutdown timed out; exiting without full cleanup", undefined, {
          event: "gateway.shutdown.timed.out.exiting.without.full.cleanup",
          outcome: "failure",
          reason: "timeout",
        });
        void (async () => {
    ...
    void (async () => {
      ...
      try {
            ...
            async () => {
              ...
              const markActiveMainSessionsForRestart = async (reason: string) => {
                ...
                try {
                  ...
                  gatewayLog.warn(
                    ...
                    undefined,
                    {
                      event: "gateway.failed.mark.interrupted.main.sessions.restart.recovery",
                      outcome: "warning",
                      reason: "failed",
                    },
                  );
              ...
              const createStillPendingDrainLogger = () =>
                setInterval(() => {
                  gatewayLog.warn(
                    ...
                    undefined,
                    {
                      event: "gateway.still.draining.active.task.s.active.embedded",
                      outcome: "warning",
                      reason: "warning",
                    },
                  );
              ...
              if (activeTasks > 0 || activeRuns > 0) {
                ...
                gatewayLog.info(
                  ...
                  undefined,
                  {
                    event: "gateway.draining.active.task.s.active.embedded.run",
                    outcome: "success",
                    reason: "completed",
                  },
                );
                if (taskBlockers) {
                  gatewayLog.warn(
                    ...
                    undefined,
                    {
                      event: "gateway.restart.blocked.active.background.task.run.s",
                      outcome: "warning",
                      reason: "blocked",
                    },
                  );
                ...
                if (restartIntent?.force) {
                  gatewayLog.warn(
                    ...
                    undefined,
                    {
                      event: "gateway.forced.restart.requested.skipping.active.work.drain",
                      outcome: "warning",
                      reason: "skipped",
                    },
                  );
                  ...
                  try {
                    ...
                    if (!runsDrain.drained && activeRuns > 0) {
                      gatewayLog.warn(
                        ...
                        undefined,
                        {
                          event: "gateway.active.embedded.run.drain.timeout.reached.aborting",
                          outcome: "warning",
                          reason: "timeout",
                        },
                      );
                  ...
                  if (tasksDrain.drained && runsDrain.drained) {
                    gatewayLog.info("all active work drained", undefined, {
                      event: "gateway.all.active.work.drained",
                      outcome: "success",
                      reason: "completed",
                    });
                  } else {
                    drainTimedOut = true;
                    gatewayLog.warn("drain timeout reached; proceeding with restart", undefined, {
                      event: "gateway.drain.timeout.reached.proceeding.restart",
                      outcome: "warning",
                      reason: "timeout",
                    });
        ...
        gatewayLog.error(`shutdown error: ${String(err)}`, undefined, {
  ...
  const request = (
    ...
    if (shuttingDown) {
      if (action === "stop" && pendingStartupRequest && !server) {
        gatewayLog.info(
          `received ${signal}; overriding pending startup restart with shutdown`,
          undefined,
          {
            event: "gateway.received.overriding.pending.startup.restart.shutdown",
            outcome: "success",
            reason: "completed",
          },
        );
      ...
      gatewayLog.info(`received ${signal} during shutdown; ignoring`, undefined, {
        event: "gateway.received.during.shutdown.ignoring",
        outcome: "success",
        reason: "completed",
      });
      return;
    ...
    gatewayLog.info(
      ...
      undefined,
      { event: "gateway.received", outcome: "success", reason: "completed" },
    );
    if (isRestart) {
    ...
    if (!server || !restartResolver) {
      ...
      void markRestartDraining().catch((err: unknown) => {
        gatewayLog.warn(
          `failed to mark gateway draining for startup restart: ${String(err)}`,
          undefined,
          {
            event: "gateway.failed.mark.gateway.draining.startup.restart",
            outcome: "warning",
            reason: "failed",
          },
        );
  ...
  const onSigterm = () => {
    gatewayLog.info("signal SIGTERM received", undefined, {
      event: "gateway.signal.sigterm.received",
      outcome: "success",
      reason: "completed",
    });
    void (async () => {
      ...
      gatewayLog.error(`failed to handle SIGTERM: ${String(err)}`, undefined, {
        event: "gateway.failed.handle.sigterm",
        outcome: "failure",
        reason: "failed",
      });
  ...
  const onSigint = () => {
    gatewayLog.info("signal SIGINT received", undefined, {
      event: "gateway.signal.sigint.received",
      outcome: "success",
      reason: "completed",
    });
    request("stop", "SIGINT");
  ...
  const onSigusr1 = () => {
    gatewayLog.info("signal SIGUSR1 received", undefined, {
      event: "gateway.signal.sigusr1.received",
      outcome: "success",
      reason: "completed",
    });
    void (async () => {
      ...
      if (!authorized) {
        ...
        if (!isGatewaySigusr1RestartExternallyAllowed()) {
          gatewayLog.warn(
            ...
            undefined,
            {
              event: "gateway.sigusr1.restart.ignored.not.authorized.commands.restart",
              outcome: "warning",
              reason: "ignored",
            },
          );
          gatewayLog.warn(
            ...
            undefined,
            { event: "gateway.onsigusr1", outcome: "warning", reason: "warning" },
          );
          return;
        }
        if (shuttingDown) {
          gatewayLog.info("received SIGUSR1 during shutdown; ignoring", undefined, {
            event: "gateway.received.sigusr1.during.shutdown.ignoring",
            outcome: "success",
            reason: "completed",
          });
          return;
      ...
      gatewayLog.error(`SIGUSR1 handler failed: ${formatErrorMessage(err)}`, undefined, {
        event: "gateway.sigusr1.handler.failed",
        outcome: "failure",
        reason: "failed",
      });
      try {
    ...
    const onIteration = createRestartIterationHook(async () => {
      ...
      if (!cronTaskDrain.drained || !cronDrain.drained) {
        gatewayLog.warn(
          ...
          undefined,
          {
            event: "gateway.cron.run.drain.timed.out.during.restart",
            outcome: "warning",
            reason: "timeout",
          },
        );
    ...
    for (;;) {
      ...
      try {
        ...
        gatewayLog.error(
          ...
          undefined,
          { event: "gateway.rungatewayloop", outcome: "failure", reason: "failed" },
        );
      }
      await new Promise<void>((resolve) => {
        restartResolver = () => {
