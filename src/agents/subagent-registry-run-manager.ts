  ...
  const waitForSubagentCompletion = async (
    ...
    const scheduleWaitRetry = (entry: SubagentRunRecord, reason: string, error?: string) => {
      ...
      log.info(
        ...
        {
          event: "agents.subagent.registry.schedulewaitretry",
          outcome: "success",
          reason: "retry",
        },
      );
    ...
    try {
      ...
      if (!task) {
        log.warn(
          ...
          {
            event: "agents.subagent.registry.failed.persist.background.task.subagent.run",
            outcome: "warning",
            reason: "failed",
          },
        );
      ...
      log.warn(
        ...
        {
          event: "agents.subagent.registry.failed.create.background.task.subagent.run",
          outcome: "warning",
          reason: "failed",
        },
      );
      ...
      for (const entry of entriesByChildSessionKey.values()) {
        ...
        void persistSubagentSessionTiming(entry).catch((err: unknown) => {
          log.warn(
            ...
            {
              event: "agents.subagent.registry.failed.persist.killed.subagent.session.timing",
              outcome: "warning",
              reason: "failed",
            },
          );
