...
| -------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `autonomous.enabled`       | `false`     | Lets OpenClaw create pending proposals from durable conversation signals when a turn ends (including failed turns — a correction given right before a timeout is still a correction). Captures both prospective rules ("from now on…") and reactive corrections ("that's not what I asked", "you're still using…"), groups them per topic (up to 3 proposals per turn), and routes each to the existing workspace skill it is about when the vocabulary overlaps. |
